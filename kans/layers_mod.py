...
class KANLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        bases = ((x_uns >= grid[:, :-1]) & (x_uns < grid[:, 1:])).to(x.dtype).to('cpu')
...
class JacobiKANLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        jacobi = torch.ones(x.shape[0], self.inputdim, self.degree + 1, device='cpu')
...
class BernsteinKANLayer(nn.Module):
    ...
    def bernstein_poly(self, x, degree):
        bernsteins = torch.ones(x.shape + (self.degree + 1, ), dtype=x.dtype, device='cpu')
        for j in range(1, degree + 1):
            for k in range(degree + 1 - j):
                bernsteins[..., k] = bernsteins[..., k].clone() * (1 - x) + bernsteins[..., k + 1].clone() * x
    ...
    def forward(self, x):
        ...
        bernsteins = bernsteins.view(-1, self.bernstein_coeffs.shape[0], self.bernstein_coeffs.shape[-1])
